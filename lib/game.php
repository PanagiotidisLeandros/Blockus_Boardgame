...
function read_g_status() {
    global $mysqli;
    $sql = 'SELECT G_status FROM game_status';
    $st = $mysqli->prepare($sql);
    $st->execute();
    $res = $st->get_result();
    $status = $res->fetch_assoc();  // Fetch a single row
    return $status ? $status['G_status'] : null;  // Return the game status or null if not found
}
// function read_p_turn() {
// 	global $mysqli;
// 	$sql = 'select * from game_status';
// 	$st = $mysqli->prepare($sql);
// 	$st->execute();
