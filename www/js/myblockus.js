...
function update_info(){
	$('#game_info').html("I am Player: "+me.piece_color+", my name is "+me.username +'<br>Token='+me.token);
...
$.ajax({
    ...
    success: function(data) {
        ...
        if (data) {
            ...
            //display the raw data in the div (as a string)
...
function display_players() {
    ...
    $.ajax({
        url: 'blockus.php/players/', // Update this to match the status route
        method: 'GET', // Ensure the method is correct (GET)
        success: function(data) {
            // Check if the data is received successfully
            if (data) {
                ...
                //display the raw data in the div (as a string)
                $('#players_result').html('<pre>' + JSON.stringify(data, null, 2) + '</pre>');
            } else {
                $('#players_result').html('<p>error</p>');
            }
        },
        error: function(jqXHR, textStatus, errorThrown) {
            console.log("AJAX Error: ", textStatus, errorThrown); // Log the error details
            $('#players_result').html('<p>Error retrieving game status .</p>');
        }
    });
    }